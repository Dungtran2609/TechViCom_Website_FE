...
const RegisterPage = () => {
  ...
  const handleRegister = async (e) => {
    ...
    if (!name.trim() || !email.trim() || !password.trim() || !confirmPassword.trim()) {
      ...
      showError('Vui lòng nhập đầy đủ thông tin!', 'Thiếu thông tin');
      return;
    }
    if (password !== confirmPassword) {
      setError('Mật khẩu không khớp!');
      showError('Mật khẩu xác nhận không đúng!', 'Lỗi xác nhận');
      return;
    }
    ...
    try {
      const res = await register({
        ...
        email,
        password,
        password_confirmation: confirmPassword, // ✅ Bắt buộc với Laravel
      });
      ...
      success(`Tài khoản của ${name} đã được tạo thành công!`, 'Đăng ký thành công');
      navigate('/login');
    } catch (err) {
      setError('Đăng ký thất bại! Kiểm tra lại email hoặc thử lại sau.');
      showError('Email đã được sử dụng hoặc có lỗi kết nối.', 'Đăng ký thất bại');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="w-full md:w-1/2 lg:w-2/5 flex flex-col items-center justify-center p-6 md:p-12">
            <div className="w-full max-w-sm">
              ...
              <form className="w-full space-y-4" onSubmit={handleRegister}>
                ...
                <button type="submit" disabled={loading} className="w-full bg-gradient-to-r from-orange-400 to-orange-500 text-white font-bold py-3 rounded-lg text-lg shadow-md hover:shadow-lg hover:from-orange-500 hover:to-orange-600 transition-all duration-300">
