  ...
  useEffect(() => {
    const loadNews = async () => {
      try {
        ...
        // Lấy mảng bài viết từ thuộc tính 'data' của đối tượng response
        const articles = response.data && Array.isArray(response.data) ? response.data : [];
        
        // Cắt lấy 3 bài viết đầu tiên trước khi cập nhật state
        setNews(articles.slice(0, 3)); 

      } catch (error) {
        console.error('Lỗi khi tải bài viết nổi bật:', error);
        setNews([]); // Đảm bảo news là mảng rỗng nếu có lỗi
      } finally {
        setLoadingNews(false); // Tắt loading sau khi hoàn tất
      }
    };
    loadNews();
